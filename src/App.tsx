        ...
        {displayGoals.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h2 className="text-2xl font-bold text-zinc-600 mb-2">
              {goals.length === 0 ? 'Nenhuma meta ainda' : 'Nenhuma meta para exibir'}
            </h2>
            <p className="text-zinc-500 mb-6">
              {goals.length === 0 ? 'Crie sua primeira meta!' : 'Ajuste os filtros no menu'}
            </p>
            {goals.length === 0 && (
              <button
                onClick={() => setShowCreateModal(true)}
                className="bg-gold-500 hover:bg-gold-600 text-black font-semibold px-6 py-3 rounded-md"
              >
                Criar Primeira Meta
              </button>
            )}
